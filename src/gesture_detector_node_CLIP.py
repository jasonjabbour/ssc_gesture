                ...
                for idx, (box, class_id) in enumerate(zip(boxes, class_ids)):
                    if class_id == 0:  # Check if the class is 'person'
                        ...
                        with torch.no_grad():
                            ...
                            # Compute similarity 
                            class_proba = logits_per_image.softmax(dim=-1)
                            class_proba = class_proba.cpu().numpy()

                            # Print probabilities for each description
                            print(f"Probabilities for Person {idx}:")
                            for desc, prob in zip(text_descriptions, class_proba[0]):
